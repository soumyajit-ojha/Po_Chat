...
def account_view(request, *args, **kwargs):
    ...
    if user.is_authenticated and user != account:
        ...
        if get_friend_request_status(sender=account, receiver=user):
            ...
            context["pending_friend_request_id"] = FriendRequest.objects.get(
                sender=account, receiver=user, is_active=True
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if (
                ...
                and account.profile_image.path != default_image_path
            ):
